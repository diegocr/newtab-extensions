 ...
 * 
 * The Original Code is National Geographic NewTab Mozilla Extension.
 * 
...
function startup(aData, aReason) {
	...
	xhr.addEventListener('load',function(){
		if(xhr.status == 200) {
			let m = xhr.responseText.match(/<div class="download_link"><a href="([^"]+\.(?:jpe?g|gif|png))"/),
			URL = m && xhr.channel.URI.resolve(m[1]) || null;
			if(!URL) {
				m = xhr.responseText.match(/\t<img src="([^"]+\.(?:jpe?g|gif|png))"/);
				URL = m && xhr.channel.URI.resolve(m[1]) || null;
			}
	...
	xhr.open('GET', 'http://www.nationalgeographic.com/photography/photo-of-the-day/', true);
	xhr.send();
}
