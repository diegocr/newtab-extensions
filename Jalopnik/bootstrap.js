 ...
 * 
 * The Original Code is Jalopnik NewTab Mozilla Extension.
 * 
...
function startup(aData, aReason) {
	...
	xhr.addEventListener('load',function(){
		if(xhr.status == 200) {
			let m = xhr.responseText.match(/<a rel="lytebox" href="([^"]+\.(?:jpe?g|gif|png))"/),
			URL = m && xhr.channel.URI.resolve(m[1].replace('\\','','g')) || null;
	...
	xhr.open('GET', 'http://jalopnik.com/wallpaper-of-the-day/', true);
	xhr.send();
}
