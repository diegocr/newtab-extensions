 ...
 * 
 * The Original Code is Earth Observatory NewTab Mozilla Extension.
 * 
...
function startup(aData, aReason) {
	...
	xhr.addEventListener('load',function(){
		if(xhr.status == 200) {
			let m = xhr.responseText.match(/<a href="view\.php\?id=\d+"><img src="([^"]+\.jpg)"/),
			URL = m && xhr.channel.URI.resolve(m[1]) || null;
			
			if(URL) {
				// URL = URL.replace('_front.jpg','_lrg.jpg');
	...
	xhr.open('GET', 'http://earthobservatory.nasa.gov/IOTD/', true);
	xhr.send();
}
